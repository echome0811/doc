...
type
  ...
  //主承销商,副承销商,担保人,原有股东,占发行量,配售比例,网上,占发行量,配售比例,网下,占发行量,配售比例
  TACBIssueInfo=Record
     S1 : Array[0..11] of String;
  End;
...
procedure TACBIssueFrm.Init(Const FileName:String);
Var
  i : Integer;
begin
...
procedure TACBIssueFrm.LoadDocData;
Var
  ID : String;
begin
...
procedure TACBIssueFrm.SetInit(Parent: TWinControl;Const FileName:String);
begin
...
procedure TACBIssueFrm.ShowLoadDocData;
Var
  i : Integer;
...
procedure TACBIssueFrm.SaveDocData;
Var
  i : Integer;
  AItem : TdxTreeListNode;
...
function TACBIssueFrm.GetMemoText: String;
begin
...
procedure TACBIssueFrm.Cbo_SectionChange(Sender: TObject);
begin
  ShowLoadDocData;
end;

procedure TACBIssueFrm.Txt_MemoChange(Sender: TObject);
begin
   if Not FLockMemoChange Then
      SaveDocData;
end;

procedure TACBIssueFrm.List_IDClick(Sender: TObject);
begin
   LoadDocData;
   ShowLoadDocData;
end;
...
procedure TACBIssueFrm.ListValueEditing(Sender: TObject;
...
procedure TACBIssueFrm.Txt_InfoChange(Sender: TObject);
begin
 if Not FLockMemoChange Then
   SaveDocData;
end;

procedure TACBIssueFrm.ListValueEdited(Sender: TObject;
...
procedure TACBIssueFrm.CreateDefauleItem;
...
begin
  ...
  CreateItem('副承销商');
  CreateItem('担保人');
  CreateItem('原有股东(1.有 0.无)');
  CreateItem('占发行量(原有股东)');
  CreateItem('配售比例(原有股东)');
  CreateItem('网上(1.有 0.无)');
  CreateItem('占发行量(网上)');
  CreateItem('配售比例(网上)');
  CreateItem('网下(1.有 0.无)');
  CreateItem('占发行量(网下)');
  CreateItem('配售比例(网下)');
...
procedure TACBIssueFrm.ListValueCustomDrawCell(Sender: TObject;
...
procedure TACBIssueFrm.Btn_RefreshClick(Sender: TObject);
begin
   LoadDocData;
   ShowLoadDocData;
end;

procedure TACBIssueFrm.Refresh(const FileName: String);
begin
   ACBIssueInfoMgr.Reback(FileName);
   Init(FileName);
end;

procedure TACBIssueFrm.SpeedButton1Click(Sender: TObject);
Var
  ID : String;
begin
...
procedure TACBIssueFrm.SpeedButton2Click(Sender: TObject);
Var
  Index : Integer;
begin
...
procedure TACBIssueFrm.Txt_IDChange(Sender: TObject);
Var
  i,j        : Integer;
...
function TACBIssueFrm.NeedSave: Boolean;
begin
   Result :=  ACBIssueInfoMgr.FNeedSaveUpload;
end;

procedure TACBIssueFrm.BeSave;
begin
  ACBIssueInfoMgr.FNeedSaveUpload:=False;
end;

procedure TACBIssueFrm.ListValueEditChange(Sender: TObject);
begin
...
procedure TACBIssueFrm.SpeedButton3Click(Sender: TObject);
Var
  ID : String;
begin
