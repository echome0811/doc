...
Type
  TfrmMSNPopUp = Class(TForm)
    ...
    Procedure imgGradientOnClick(Sender: TObject);
  protected
...
Try
  ...
  imgGradient.OnClick := self.imgGradientOnClick;
...
Begin
  ...
  // Modify by JoySun 2005/10/27 for not disappear the popup window
...
procedure TfrmMSNPopUp.imgGradientOnClick(Sender: TObject);
begin
  // close the popup window on clicking the background
  //JoySun
  {If Assigned(Self.ParentMSNPopup.FOnClick) Then
   Begin
    CanClose := False;
