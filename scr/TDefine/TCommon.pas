...
interface
  Uses Forms,Windows,Controls,Dialogs,Classes,SysUtils,fileCtrl,Math,TDeclar,
  Registry,inifiles
  ,WinInet,URLMon, ShellApi //add by wangjinhua Doc 4.13 - Problem(20100221)
  ,Messages //add by wangjinhua 20090626 Doc4.3
  ,IdHTTP, IdTCPClient,TLHelp32; //uHttpFun ,
...
const
  ...
  type TTrancsationRec = packed record
    ID:shortstring; //巨絏
    Op:shortstring;  //┮巨
    OpTime:ShortString;//巨丁
    Operator:ShortString;//巨
    IDName:ShortString;//絏嘿
    ModouleName:ShortString;//巨家遏嘿
    ModouleNameEn:ShortString;//巨家遏璣ゅ嘿
    BeSave:Boolean;//琌砆玂
  end;
  TTrancsationRecs = Array of TTrancsationRec;

  type TUser = packed record
    ID:shortstring; //用户账号
    Password:shortstring;  //用户密码
    LookPurview:array[0..MaxP-1] of boolean;  //模块查看权限
    EditPurview:array[0..MaxP-1] of boolean;  //模块编辑权限
    SuperUser:Boolean;  //是否为超级用户
  end;
  type TUsers = Array of TUser;
  type TUserOpType = (opAdd,opEdit,opDelete,opLookUp,opNone);
//--
//const
...
const NullStr='NullString';//add by wangjinhua Doc 4.13 - Problem(20100221)

Type _cStrLst = Array of ShortString;
Type _cStrLst2 = Array of String;

 //--Doc3.2.0需求1 huangcq080923 add-->
  //M:market_db; P:publish_db
  //M_OutPassaway_P_All:M扣除Passaway外的其他txt文件,P取所有
 Type TxtFileScope_MP=(M_All_P_All,M_None_P_All,M_OutPassaway_P_All,M_OutPassaway_P_None,M_All_P_None);
 //<--Doc3.2.0需求1 huangcq080923 add--
...
Type
  ...
  TOnGetFile=Procedure(FileName:String) of Object;
...
//add by wangjinhua Doc 4.13 - Problem(20100221)
//KillTask('HotMsg.exe');
function KillTask(ExeFileName: string): Integer;
function CheckTask(ExeFileName: string): Boolean;
function GetTextByTs(AFile:string; var AText:string):boolean;
function SetTextByTs(AFile,AText:string):boolean;
function GetValueByFiledName(aFiledName:string;ts:TStringList;var aOutValue:string):integer;
procedure TsIniReadSectionValues(aOption:integer;tsInput:TStringList;aSection:string;var tsOutput:TStringList);
function GetFieldValue(aInput:string;var aField,aValue:string):boolean;
function GetFieldValueEx(aInput,aSep:string;var aField,aValue:string):boolean;
function IsSecLine(aLine:string):boolean;
procedure WriteFileLine(sFile : string; sLine : string);
procedure WriteLineForApp(sLine : string;aTag:ShortString='');
function FmtDt8(aDate:TDate):string;
function FmtDt10(aDate:TDate):string;
function  GetWebPageText(sUrl:string;var sErrMsg:string):string;
function DownloadFile(AUrl:String; var AResultStr,AErrMsg: string):Boolean;
{function GetRtf_Date_Title(aStr: String; var aRtf,aDate,aTitle:string;
  var aDocType,aStrikeType:byte): Boolean;}
function GetRtf_Date_Title(aStr,aTxtFile: String;
  var aRtf,aDate,aTitle:string;
  var aDocType,aStrikeType:byte): Boolean;
function GetRtf_Date_Title3(aStr: String;
  var aRtf,aDate,aTitle:string): Boolean;
function GetRtf_Date_Title4(aStr,aStr2: String;
  var aRtf,aDate,aTitle:string;
  var aDocType,aStrikeType:byte): Boolean;
function GetRtf_Date_Title5(aStr: String;
  var aRtf,aDate,aTitle:string): Boolean;
function CreateAnEmptFile(aFile:string):boolean;
//function GetHtmlTextWithIdAnHttp(IdHttp:TIdHttp;const TxtUrl: String;var OutText,ErrMsg:String;TimeOut:Integer=60):Boolean;
//--

function ToAlignString(const str: string; len: Integer): string;
function PosEx(aSub,aSrc:string):integer;

//add by wangjinhua Doc 4.13 - Problem(20100527)
function SetFullUrl(AHomeUrl,ABaseUrl,ASubUrl:string):string;
function GetUrlType(AUrl:String):TUrlType;
procedure DeleteIECache;
//function GetStr(StartTag,EndTag:string;var ASource :String;IncludeTag:Boolean=false;RmvTag:Boolean=true):string;
function GetStr(StartTag,EndTag:string;var ASource :String;IncludeTag:Boolean=false;RmvTag:Boolean=true):string;
function GetStr2(StrSource,StrBegin,StrEnd:string):string;
...
//根據設定檔更新INI的某些字段,因為如果有新的字段要增加,可透過此方式臨時加入
Procedure ReWriteUpdateSetIni(Path:String);

Procedure SetTickCount();
Function ResetTickCount():Boolean;

Function  TwoFileIsTheSame(file1,file2:String):Boolean;

function  CoCreateGuid(var tGUIDStructure:TGUID):longint; stdcall ;external 'ole32.dll';

Function  DoublePointCount(v:Double):Integer;

function  IsDate(StrDate:ShortString):Boolean;
Function  StrToDate2(StrDate:ShortString):TDate;
function DateStr8ToDate(Str: String): TDate; //DateStr8:yyyymmdd --DOC4.0.0—N004 huangcq090317 add
function  Round2(x : extended) : Int64;
Function  IsInteger(pStr:PChar):Boolean;
Function  IsInteger2(pStr:PChar):Boolean;
Function  IntToBool(nInt:Integer):Boolean;
Function  BoolToInt(nBool:Boolean):Integer;
Function  BoolToStr(nBool:Boolean):String;
function ComStrToBool(aInput:string):Boolean;
Function  IsQianlongPath(Path:ShortString):Boolean;
function  IsFileInUse(fName : ShortString) : boolean;
function  IsFileInExec(fName : ShortString) : boolean;

Procedure SearchAllFolders(DirName : shortString;Var Files :_cStrLst);

Procedure FolderAllFiles(DirName : shortString;SpecExt:ShortString;
               Var Files :TStringList;IncludeSubFolder:Boolean=True);Overload;
Procedure FolderAllFiles(DirName : shortString;SpecExt:ShortString;
               Var Files :_cStrLst;IncludeSubFolder:Boolean=True);Overload;
Procedure FolderAllFiles(DirName : shortString;SpecExt:ShortString;
              OnGetFile:TOnGetFile;IncludeSubFolder:Boolean=True);Overload;
Procedure GetTxtFilesFromDblst(DirName : shortString;Var Files :_cStrLst;TxtScope:TxtFileScope_MP);//--Doc3.2.0需求1 huangcq080923 add--              

Function  FolderExistFiles(DirName : shortString;SpecExt:ShortString):Boolean;
procedure FolderFileSize(DirName : shortString;Var Size:LongInt);
procedure Deltree(DirName : shortString;IncludeSubFolder:Boolean=True;DelFolderName:Boolean=False);
procedure DelAllFiles(DirName : shortString;IncludeSubFolder:Boolean=True);
Function  TheSameDateTime(SFile1,DFile2:ShortString):Boolean;
function  GetShtPathName(const PathName:ShortString):ShortString;
function  GetWindowsDir:ShortString;
Function  GetWinSysPath():ShortString;
Function  GetWinTempPath():ShortString;
Function  CFPower_FToI(v:Double;vPower:Integer):Longint;
Procedure Mkdir_Directory(Path:ShortString);
Procedure DoPathSep(Var path:ShortString);
Procedure DoPathSep2(Var path:string);
Procedure ClearPathSep(Var path:ShortString);
Function  Get_DateSep():ShortString;
Procedure SetIniFormatCache(inifile:PChar;IniLst:TList=Nil);
Procedure FreeIniCache(IniLst:TList=Nil);
Function  IniCacheCount(IniLst:TList=Nil):Integer;
Function  GetIniCache(SectionName:PChar;
                      keyName:PChar;
                      DefaultS:PChar;IniLst:TList=Nil):ShortString;

function  GetIniFile(SectionName:PChar;
                    keyName:PChar;
                    DefaultS:PChar;
...
function  CBIG5toGB(value: String;isSys:Boolean=True): String;
function  isGB(value: ShortString): Boolean;
function  isBIG5(value: ShortString): Boolean;
Procedure SetLanguage(value:ShortString);
function  SysisBIG5(): Boolean;

Function  ReadRegStr(AppName,AppKey:ShortString):ShortString;
Function  ExistReg(AppName:ShortString):Boolean;
Procedure WriteRegStr(AppName,AppKey,AppValue:ShortString);
Procedure DelRegStr(AppName:ShortString);

function GetWin_ProxyEnable(var aProxyEnable:boolean;
  var aProxSvr:ShortString;var aProxPort:integer):boolean;


Function  DoStrArray(Str:ShortString;sep:ShortString): _cStrLst;
Function  DoStrArray2(Str:String;sep:String): _cStrLst2;
Function  DoStrArray2_2(Str:String;sep:String): _cStrLst2;
Function  DelStrSep(Str:ShortString;sep:ShortString): ShortString;


Function  GetBDESessionPath(TempFolder:ShortString):ShortString;
Function  Get_GUID():ShortString;
Function  Get_GUID8():ShortString;

Procedure Check_Num(Text:ShortString;Var key : Char);
Procedure Check_Num_NoNeg(Text:ShortString;Var key : Char);
Procedure Check_Num_NoPoint(Text:ShortString;Var key : Char);

Procedure SortFileLst(Var BufferGrid:_CStrLst);    //0 递增 1 递减
Procedure SortDateLst(Var BufferGrid:Array of TDate);    //0 递增 1 递减
...
Function  StrToDateTime2(StrDate:ShortString):TDateTime;

function fCopy(str:string;MaxLength:Integer):string;

function Rounding(Num: Double; DecPt: integer): Double;

function Gb2Big5(Str:string):string;
function Big52Gb(Str:string):string;

Function GetFileVer(aFileName:String):String;

Function GetLocaleInformation(Flag: Integer): String;
//	　參數"Flag"可以取下列值:
//	LOCALE_NOUSEROVERRIDE { do not use user overrides }
//	LOCALE_USE_CP_ACP { use the system ACP }
//	LOCALE_ILANGUAGE { 語言代號 0804->GB2312 0404->BIG5}
...
implementation
   Uses {ShellAPI,Tlhelp32,;}ComObj, TDocMgr;
...
var
  ...
  IniFileRecLst : TList;
  FTickCount : DWord;
  //BIG2GBInstance,GB2BIGInstance: THandle;
  //BIGTAB,GBTAB, DupCode, DupData: TResourceStream;
...
var
  vCurDir:String;
  vBaseUrl:ShortString;
  i,iPos,j:Integer;
begin
  Result := '';
  vCurDir:=ExtractFilePath(ABaseUrl);
  case GetUrlType(ASubUrl) of
    urlFull: Result := ASubUrl;
    urlAfterHome: Result := AHomeUrl + ASubUrl;
    urlCurDir: Result := vCurDir + ASubUrl;
    urlUpGrade:begin
      i:=0;
       while true do
       begin
         Application.ProcessMessages;
         iPos:=Pos(UpGradeFlag,ASubUrl);
         if iPos=0 then break;
         ASubUrl:=Copy(ASubUrl,iPos+Length(UpGradeFlag),Length(ASubUrl)-iPos-Length(UpGradeFlag)+1 );
         Inc(i);
       end;
       vBaseUrl:=ABaseUrl;
       while i>0 do
       begin
         ClearPathSep(vBaseUrl);
         iPos:=0;
         for j:=Length(vBaseUrl) downto 1 do
         begin
           Application.ProcessMessages;
           if vBaseUrl[j]='/' then
           begin
         ...
         if iPos<>0 then
           vBaseUrl:=Copy(vBaseUrl,1,iPos);
         i:=i-1;
       end;
       Result := vBaseUrl + ASubUrl;
    end;
  end;
end;
...
begin
  Result := urlNone;
  AUrl := Trim(AUrl);
  if (Pos('http:',AUrl) =1 ) or
     (Pos('https:',AUrl)=1 )  then
    Result := urlFull
  else if Pos('/',AUrl) =1 then
    Result := urlAfterHome
  else if Pos('../',AUrl)=1 then
    Result:=urlUpGrade
  else
...
begin
  ...
  tmpStr:= copy(ASource,iStartPos,Length(ASource)-iStartPos-length(StartTag)+1);
  iEndPos := AnsiPos(LowerCase(EndTag),LowerCase(tmpStr));
  if iEndPos<=0 then exit;
  result := copy(tmpStr,1,iEndPos + Length(EndTag) - 1);
end;
...
begin
  ...
  begin
    str:=result;
    if not IncludeTag then
    begin
      str:=StartTag+str+EndTag;
    end;
    ASource:=StringReplace(ASource,str,'',[]);
  end;
end;
{function GetStr(StartTag,EndTag:string;var ASource :String;IncludeTag:Boolean=false;RmvTag:Boolean=true):string;
var
...
begin
  ...
  if IncludeTag then
    result := copy(ASource,iStartPos,iEndPos + Length(EndTag) - 1)
  else
...
var s:string;
begin
//Result:=GetStr(StrSource,StrBegin,StrEnd);
Result:=GetStr(StrBegin,StrEnd,StrSource,false);
if (Copy(Result,1,2) =#13#10) then
  result := Copy(Result,3,Length(result)-2);
if (Copy(Result,Length(result)-1,2) =#13#10) then
  result := Copy(Result,1,Length(result)-2);
...
var
  i,j:Integer;
begin
  i := Low(OpTrancsationRecs);
  while i < High(OpTrancsationRecs) do
  begin
    if OpTrancsationRecs[i].ID = '' then
    begin
      for j := i to High(OpTrancsationRecs) - 1 do
      begin
        OpTrancsationRecs[j].ID := OpTrancsationRecs[j+1].ID ;
        OpTrancsationRecs[j].Op := OpTrancsationRecs[j+1].Op;
        OpTrancsationRecs[j].OpTime := OpTrancsationRecs[j+1].OpTime;
        OpTrancsationRecs[j].Operator := OpTrancsationRecs[j+1].Operator;
        OpTrancsationRecs[j].ModouleName := OpTrancsationRecs[j+1].ModouleName;
        OpTrancsationRecs[j].ModouleNameEn := OpTrancsationRecs[j+1].ModouleNameEn;
        OpTrancsationRecs[j].BeSave := OpTrancsationRecs[j+1].BeSave;
      end;
      SetLength(OpTrancsationRecs,Length(OpTrancsationRecs) -1);
    end else
      Inc(i);
  end;
end;
...
var
  i:Integer;
begin
  for i := Low(OpTrancsationRecs) to High(OpTrancsationRecs) do
     with OpTrancsationRecs[i] do
      begin
        ID := '';
        Op := '';
        OpTime := '';
...
begin
  for i := Low(OpTrancsationRecs) to High(OpTrancsationRecs) do
  begin
    if OpTrancsationRecs[i].ModouleNameEn = AModouleEn then
    begin
      if SaveRec then
      begin
        OpTrancsationRecs[i].ModouleName := AModoule;
        //WriteARec(OpTrancsationRecs[i]);
        SendLogMsg(FAppParam.DocServer, 8090, OpTrancsationRecs[i]);
      end;
      with OpTrancsationRecs[i] do
      begin
        ID := '';
...
begin
  ...
  begin
    for i:=Low(OpTrancsationRecs) to High(OpTrancsationRecs) do
    begin
      if (OpTrancsationRecs[i].ID=AnID) and
         ( (OpTrancsationRecs[i].Op='Add') or
           (OpTrancsationRecs[i].Op='Upt')
         ) then  exit;
    end;
  end;
  vOpTime := FormatDateTime('yyyy-mm-dd hh:mm:ss',now);
  SetLength(OpTrancsationRecs,Length(OpTrancsationRecs) + 1);
  vHigh := High(OpTrancsationRecs);
  OpTrancsationRecs[vHigh].ID := AnID;
  OpTrancsationRecs[vHigh].Op := AnOp;
  OpTrancsationRecs[vHigh].OpTime := vOpTime;
  OpTrancsationRecs[vHigh].Operator := AnOperator;
  OpTrancsationRecs[vHigh].ModouleName := AnModouleName;
  OpTrancsationRecs[vHigh].ModouleNameEn := AnModouleNameEn;
  OpTrancsationRecs[vHigh].BeSave := false;
end;

procedure WriteARecToFile(ARec:TTrancsationRec;aFile:string);
var
  vTrancsationRecs:TTrancsationRecs;
  f:file of TTrancsationRec;
  vFile:String;
begin
  vFile := aFile;
  Mkdir_Directory(ExtractFilePath(vFile));
  SetLength(vTrancsationRecs,0);
  SetLength(vTrancsationRecs,Length(vTrancsationRecs) + 1 );
  vTrancsationRecs[High(vTrancsationRecs)] := ARec;
  try
    try
      AssignFile(f,vFile);
      FileMode:=2;
      if not FileExists(vFile) then
        ...
        Reset(f);
      Seek(f,FileSize(f));
      BlockWrite(f,vTrancsationRecs[0],Length(vTrancsationRecs));
    except
      on e:Exception do
        e := nil;
    end;
...
var vFile:String;
begin
  vFile := ExtractFilePath(ParamStr(0))+  SubDir + FormatDateTime('yyyymmdd',date) +  '.log';
...
function CreateAnEmptTrancsationDatFile(aFile:string):boolean;
var sPath:String; f:file of TTrancsationRec;
begin
  result:=false;
...
function AddTrancsationDatToFile(ALogFile,aDstFile:String):boolean;
var
  f,f2:file of TTrancsationRec;
  r:array of TTrancsationRec;
  Remain,Remain2:integer;
begin
    result:=false;
...
begin
  vFile := ExtractFilePath(ParamStr(0))+
           SubDir + FormatDateTime('yyyymmdd',date) +  '.log';
  vMsg := '[' + AUser + '] ' +
          '[' + AOP + '] ' +
          '[' + APage + '] ' +
          '[' + FormatDateTime('hh:mm:ss',Now) + '] ';
  AddTextToFile(vFile,vMsg);
end;

procedure AddTextToFile(AFile,AMsg:String);
var fp : TextFile;
begin
    Mkdir_Directory(ExtractFilePath(AFile));
    try
        AssignFile(fp, AFile);
...
begin
  ...
  StrLst.Clear;
  try
    StrLst.Add('UPTLOGRECSFILE:');
    StrLst.Add(ARec.ID);
    StrLst.Add(ARec.Op);
    StrLst.Add(ARec.OpTime);
    StrLst.Add(ARec.Operator);
    StrLst.Add(ARec.ModouleName);
    StrLst.Add(ARec.ModouleNameEn);
    if ARec.BeSave then
      StrLst.Add('1')
    else
      StrLst.Add('0');
...
var sFile,sPath : string;
    CLogPath:string;
begin
    CLogPath  := ExtractFilePath(ParamStr(0))+'TestLog\';
    if not DirectoryExists(CLogPath) then Exit;
    sPath:=Format('%s%s\',[CLogPath,
        FormatDateTime('yyyymmdd',now)  ]);
    if not DirectoryExists(sPath) then ForceDirectories(sPath);
    sFile := sPath+Format('%s%s.log',[aTag,FormatDateTime('yyyymmdd_hh_mm',now)]);
...
begin
  ...
  if aGuid='' then
    exit;
  sTemp1:=ExtractFilePath(aLogFile);
  if not DirectoryExists(sTemp1) then
    exit;
  ts:=TStringList.create;
  try
    if FileExists(aLogFile) then
...
// ゅン獺
function GetFileInfo(const FileName: string;aType:integer; var FileSize: Int64; var FileTime: TDateTime): Boolean;
var
  Handle: THandle;
...
begin
  ...
  begin
    ...
    begin
      ...
      if aType=1 then
        FileTime := FileTimeToDateTime(FindData.ftCreationTime)
      else if aType=2 then
        FileTime := FileTimeToDateTime(FindData.ftLastAccessTime)
      else
...
begin
  Result := 0;
  GetFileInfo(FileName,0, Size, Result);
end;
...
procedure DoStrArrayTs(Str:String;sep:String; var tsRst:TStringList);
var ts:TStringList;
    aSrc:string;  i:integer;
begin
  try
    tsRst.clear;
    ts:=TStringList.create;
    aSrc:=Str;
    aSrc:=stringreplace(aSrc,sep,#13#10,[rfReplaceAll]);
    ts.text:=aSrc;
    if copy(Str,length(Str)-length(sep)+1,length(sep))=sep then
      ts.add('');
    for i:=0 to ts.count-1 do
     tsRst.Add(ts[i]);
  finally
...
begin
  ...
  try
    result:=false;
    ts:=nil;
    if not DirectoryExists(ExtractFilePath(AFile)) then exit;
    ts:=TStringList.create;
    ts.text:=AText;
    ts.SaveToFile(AFile);
    result:=True;
...
function GetValueByFiledName(aFiledName:string;ts:TStringList;var aOutValue:string):integer;
  var ix:integer; xstr,xstr1,xstr2:string; 
  begin
    result:=-1;
    aOutValue:='';
    xstr:=aFiledName+'=';
    ...
    begin
      if Pos(xstr,ts[ix])=1 then
      begin
        xstr1:=ts[ix];
        aOutValue:=StringReplace(xstr1,xstr,'',[]);
        result:=ix;
        break;
      end;
    end;
...
begin
  ...
  begin
    aField:=Copy(aInput,1,i-1);
    aValue:=Copy(aInput,i+length(aSep),Length(aInput));
    result:=true;
  end;
end;

  procedure TsIniReadSectionValues(aOption:integer;tsInput:TStringList;aSection:string;var tsOutput:TStringList);
  var i,j:integer; sLine,sIniSection,sField,sValue:string;
  begin
    tsOutput.clear;
    if (aSection='') or (tsInput=nil) then
      exit;
    sIniSection:='['+aSection+']';
    if Pos(sIniSection,tsInput.text)<=0 then
      Exit;
    for i:=0 to tsInput.Count-1 do
    begin
      if SameText(tsInput[i],sIniSection) then
      begin
        for j:=i+1 to tsInput.count-1 do
        begin
          if IsSecLine(tsInput[j]) then
            break
          else begin
            if Trim(tsInput[j])<>'' then
            begin
              if aOption=1 then
              begin
                GetFieldValue(tsInput[j],sField,sValue);
                tsOutput.Add(sField); 
              end
...
function   GetWebPageText(sUrl:string;var sErrMsg:string):string;
  ...
  begin
      ...
      //   {不将Internet连接的资料写入Cache中}
    try
    try
      sErrMsg := '';
      hInternet   :=   InternetOpen(pchar('KaiLongHttpDwn'),
      ...
      begin   
          ...
          begin   
              ...
              begin
                  ...
                  InternetReadFile(hOpenUrl,   @lpBuffer,   sizeof(lpBuffer),   lpdwNumberOfBytesRead)   ;
...
{
function GetRtf_Date_Title(aStr: String;
  var aRtf,aDate,aTitle:string;
  var aDocType,aStrikeType:byte): Boolean;
var sTemp,sTempDate,sTemp2:String;
    StrLst:_cStrLst;
begin
  aRtf:=''; aDate:=''; aTitle:='';
  aDocType:=0; aStrikeType:=0;
  result :=false;
  try
    if(Length(aStr)=0)then exit;
    aRtf:=GetStrOnly2('=',',',aStr,true);
    aRtf:=Copy(aRtf,2,Length(aRtf)-2);
    sTempDate:=GetStrOnly2('_','_',aRtf,true);
    sTemp2:=GetStrOnly2(sTempDate,'.rtf',aRtf,false);
    StrLst:=DoStrArray(sTemp2,'_');
    if (Length(StrLst)=3) and
       (StrLst[0]<>'') and
       (StrLst[1]<>'') and
       (StrLst[2]<>'') then
    begin
      aDocType:=StrToInt(StrLst[1]);
      aStrikeType:=StrToInt(StrLst[2]);
    end;
    sTempDate:=Copy(sTempDate,2,Length(sTempDate)-2);

    aDate:=Copy(sTempDate,1,4)+'-'+Copy(sTempDate,5,2)+'-'+Copy(sTempDate,7,2);
    aTitle:=GetStrOnly2(',','/'+aDate,aStr,false);
    result :=true;
  except
  end
end;  }

procedure GetParams_FileName_Sq_TheType_StrikeType(aInput:string;
  var aFileName,aSq,aTheType,aStrikeType:string);
begin
  aFileName:=GetStrOnly2('FileName=','Sq=',aInput,false);
  aFileName:=trim(aFileName);
  aSq:=GetStrOnly2('Sq=','TheType=',aInput,false);
  aSq:=trim(aSq);
  aTheType:=GetStrOnly2('TheType=','StrikeType=',aInput,false);
  aTheType:=trim(aTheType);
  aStrikeType:=GetStrOnly2('StrikeType=','DocDate=',aInput,false);
  aStrikeType:=trim(aStrikeType);
end;
...
var sTemp,sTempDate,sTemp2:String;
    StrLst:_cStrLst;
    f:TStringList;
    FileName2,sId,ssFileName,ssSq,ssTheType,ssStrikeType,sTemp4,sTemp3:string;
begin
  aRtf:=''; aDate:=''; aTitle:='';
  aDocType:=0; aStrikeType:=0;
  result :=false;
  try
    ...
    begin
      ...
      try
        f.LoadFromFile(aTxtFile);
        sTemp3:=Format('<ID=%s FileName=%s',[
         sId,FileName2] );
       sTemp4:=GetStrOnly2(sTemp3,'</ID>',f.text,true);
       GetParams_FileName_Sq_TheType_StrikeType(sTemp4,
        ssFileName,ssSq,ssTheType,ssStrikeType);
        if Length(ssTheType)>0 then
          aDocType:=StrToInt(ssTheType);
        if Length(ssStrikeType)>0 then
          aStrikeType:=StrToInt(ssStrikeType);
      finally
        if Assigned(f) then
          f.Free;
      end;
...
function CreateAnEmptFile(aFile:string):boolean;
var sPath:String; ts:TStringList;
begin
  sPath:=ExtractFilePath(aFile);
  if not DirectoryExists(sPath) then
  begin
...
begin
  ...
  try
    if(Length(aStr)=0)then exit;
    sId:=GetStrOnly2('FileName=','_',aStr,true);
    sId:=Copy(sId,1,Length(sId)-1);
    sId:=StringReplace(sId,'FileName=','',[rfReplaceAll]);
    
    aRtf:=GetStrOnly2('FileName=','.rtf',aStr,true);
    aRtf:=StringReplace(aRtf,'FileName=','',[rfReplaceAll]);

    sTemp:=GetStrOnly2('TheType=','StrikeType=',aStr,true);
    sTemp:=StringReplace(sTemp,'TheType=','',[rfReplaceAll]);
    sTemp:=StringReplace(sTemp,'StrikeType=','',[rfReplaceAll]);
    sTemp:=Trim(sTemp);
    if sTemp<>'' then
    aDocType:=StrToInt(sTemp);

    sTemp:=GetStrOnly2('StrikeType=','DocDate=',aStr,true);
    sTemp:=StringReplace(sTemp,'StrikeType=','',[rfReplaceAll]);
    sTemp:=StringReplace(sTemp,'DocDate=','',[rfReplaceAll]);
    sTemp:=Trim(sTemp);
    if sTemp<>'' then
    aStrikeType:=StrToInt(sTemp);

    sTemp:=GetStrOnly2('DocDate=','>',aStr,true);
    sTemp:=StringReplace(sTemp,'DocDate=','',[rfReplaceAll]);
...
{begin
  DeleteIECache;
  Result:=GetHtmlByApi(AURL,AResultStr,AErrMsg);
  DeleteIECache;
end;}
...
{for i := 1 to Length(str) do
  if Ord(str[i]) < 256 then Inc(n); }
if n > 0 then s := StringOfChar(' ', n);
if Len <= 0 then Result := s + str
else Result := str + s;
end;
...
function GetHtmlTextWithIdAnHttp(IdHttp:TIdHttp;const TxtUrl: String;var OutText,ErrMsg:String;TimeOut:Integer=60):Boolean;
begin
  //Result:=GetHtmlTextWithIdHttp(IdHttp,TxtUrl,OutText,ErrMsg,TimeOut);
end;}
//--
...
begin
  Result := 'File did not contain version info';
try
  if GetFileVersion(aFileName,fileinfo) then
    Result := GetProductVerStr(fileinfo);
...
Try
    ...
    Begin
       DateSeparator:='-';
       Result := TryStrToDate(StrDate, ADate);
       Exit;
    End;
    if Pos('/',StrDate)>0 Then
    Begin
...
Try
   ...
   Begin
      ...
      Count := f.ReadInteger('UPDATE','COUNT',0);
      for i:=1 to Count do
      Begin
         Str := f.ReadString('UPDATE',IntToStr(i),'');
         if Length(Str)>0 Then
         Begin
           StrLst := DoStrArray2(Str,',');
           if High(StrLst)=3 Then
           Begin
              f2 := TiniFile.Create(Path+StrLst[0]);
              f2.WriteString(StrLst[1],StrLst[2],StrLst[3]);
              f2.Free;
              f2 := nil
           End;
         End;
...
function Rounding(Num: Double; DecPt: integer): Double;
var
FirstNum, SecondNum, PoNum: Double;
i: integer;
begin
FirstNum := Int(Num);
SecondNum := Frac(Num);
...
PoNum := Int(PoNum * 10);
if Num > 0 then
SecondNum := Int((SecondNum * PoNum) + 0.51) / PoNum
else
SecondNum := Int((SecondNum * PoNum)- 0.51) / PoNum;
Rounding := FirstNum + SecondNum;
end;
...
var
  ws:WideString;
begin
  Str:=Copy(Str,1,MaxLength); //表示你的欄位最長為25碼
  ws:=Str; //轉成WideString
  Result:=Copy(ws,1,Length(ws)); //再截一次即可
end;
...
Try
    ...
    if FileSize(f)<>FileSize(f2) then
    Begin
        CloseFile(f);
        CloseFile(f2);
        Exit;
...
var
  //排序用
  SortSelectGrid,SelectGrid  :Array of TDate;
...
Begin
  ...
  SetLength(SortSelectGrid,High(BufferGrid)+1);
  SetLength(SelectGrid,High(BufferGrid)+1);

  //取出所有的 Record 准备排序
  for i:=0 to High(BufferGrid) do //取出筛选的比数
      sortSelectGrid[i]:=BufferGrid[i];
  ...
  lHValue :=Count-1;
    ...
    repeat
        ...
        Begin
            ...
            lTemp := sortSelectGrid[lLoop1];

            lHold := lLoop1;
            while sortSelectGrid[lHold - lHValue]
                        > lTemp do
            Begin
                 sortSelectGrid[lHold] := sortSelectGrid[lHold - lHValue];
                 lHold := lHold - lHValue;
                 If lHold < lHValue Then break;
            End;
            sortSelectGrid[lHold] := lTemp;

        end;
    Until lHValue = 0;

    for i:=0 to High(BufferGrid) do //取出排序结果
       BufferGrid[i] := sortSelectGrid[i];
...
var
  //排序用
  SortSelectGrid,SelectGrid  :_CStrLst;
  lLoop1,lHold,lHValue       :Longint;
  lTemp                      :ShortString;
  i,Count :Integer;
Begin
...
begin
   ...
   Result := False;   //by leon 0808  //Doc_01-DOC3.0.0需求4-leon-08/8/14-add;
   //  Result := True;

    tStr := StrPas(pStr);
    //tStr := DelStrSep(tStr,'-');
    //tStr := DelStrSep(tStr,'%');
   // tStr := DelStrSep(tStr,'、'); //CBStrike3Frm2.0.0.0-Doc2.3.0.0-需求6-libing-2007/09/20-添加
   // tStr := DelStrSep(tStr,'.');
    Try
       if TextToFloat(PChar(tStr), td, fvExtended) then
          Result := True;
    Except
       //Result:=false;
    End;
...
Begin
     ...
     Tmp := GetWinTempPath;
     if Length(Tmp)=0 Then Tmp := 'C:\Temp';
     DoPathSep(Tmp);
     Tmp := Tmp+TempFolder;
     Mkdir_Directory(Tmp);
     Result := Tmp;
...
Begin
     ...
     DoPathSep(Path);
     if (Not DirectoryExists(Path+ 'Program\')) or (Length(Path)=0) Then
        Result := false
     else
        Result := True;
...
begin
  ...
  if not FileExists(fName) then
   exit;
  HFileRes := CreateFile(pchar(fName+''), GENERIC_EXECUTE,0, nil, OPEN_EXISTING,FILE_ATTRIBUTE_NORMAL, 0);
  Result := (HFileRes = INVALID_HANDLE_VALUE);
  CloseHandle(HFileRes);
...
begin
  ...
  DoPathSep(DirName);
  r := FindFirst(DirName+'*.*', faDirectory, DirInfo);
  while r = 0 do
  begin
     if ((DirInfo.Attr and FaDirectory) = FaDirectory) and (DirInfo.Name<>'.')
        and (DirInfo.Name<>'..') then
       SearchAllFolders(DirName + DirInfo.Name,Files);
    if ((DirInfo.Attr and FaDirectory = FaDirectory) or
...
begin
  ...
  begin
    if IncludeSubFolder Then
     if ((DirInfo.Attr and FaDirectory) = FaDirectory) and (DirInfo.Name<>'.')
        and (DirInfo.Name<>'..') then
       FolderAllFiles(DirName + DirInfo.Name,SpecExt,OnGetFile,IncludeSubFolder);
    if ((DirInfo.Attr and FaDirectory <> FaDirectory) and
      (DirInfo.Attr and FaVolumeId <> FaVolumeID)) then
    Begin
        if (UpperCase(ExtractFileExt(DirInfo.Name))=SpecExt) or (SpecExt='*.*') Then
        Begin
          if Assigned(OnGetFile) Then
             OnGetFile(DirName+DirInfo.Name);
        End;
    End;
    r := FindNext(DirInfo);
  end;
...
Procedure FolderAllFiles(DirName : shortString;SpecExt:ShortString;Var Files :TStringList;IncludeSubFolder:Boolean=True);
var
  DirInfo: TSearchRec;
  r,i : Integer;
begin
  ...
  begin
    if IncludeSubFolder Then
     if ((DirInfo.Attr and FaDirectory) = FaDirectory) and (DirInfo.Name<>'.')
        and (DirInfo.Name<>'..') then
       FolderAllFiles(DirName + DirInfo.Name,SpecExt,Files,IncludeSubFolder);
    if ((DirInfo.Attr and FaDirectory <> FaDirectory) and
      (DirInfo.Attr and FaVolumeId <> FaVolumeID)) then
    Begin
        if (UpperCase(ExtractFileExt(DirInfo.Name))=SpecExt) or (SpecExt='*.*') Then
...
Procedure FolderAllFiles(DirName : shortString;SpecExt:ShortString;Var Files :_cStrLst;IncludeSubFolder:Boolean=True);
var
  DirInfo: TSearchRec;
  r,i : Integer;
begin
...
//---Doc3.2.0需求1 huangcq080923 add--------------------------->
//-该函数的DirName是market_db的上级目录
Procedure GetTxtFilesFromDblst(DirName : shortString;Var Files :_cStrLst;TxtScope:TxtFileScope_MP);

  Function GetDbLstName(pFileFolderName:string):String;
  var
    f:TIniFile;
  begin
    Result:='';
    f:=TIniFile.Create(ExtractFilePath(Application.ExeName)+'Setup.ini');
    if LowerCase(pFileFolderName)='market_db' then
      Result:=f.ReadString('DBLSTNAME','market_db','dblst311.lst')
    else
      if LowerCase(pFileFolderName)='publish_db' then
        Result:=f.ReadString('DBLSTNAME','publish_db','dblst.lst');
    f.Free;
  end;

  Procedure GetTxtFiles(DblstFileName:ShortString;Var Files:_cStrLst);
  var
    ATxtFile:shortString;
    f:TIniFile;
    i,j:Smallint;
    cStrLst:_cstrLst;
  begin
    f:=TIniFile.Create(DblstFileName);
    ATxtFile:=f.ReadString('dblst','filename','');
    f.Free;
    cStrLst:=DoStrArray(ATxtFile,',');
    for i:=0 to high(cStrLst) do
    begin
      if not FileExists(ExtractFilePath(DblstFileName)+cStrLst[i]) then
        continue;
      //cStrLst[i]:=ExtractFilePath(DblstFileName)+cStrLst[i];
      j:=High(Files)+1;
      SetLength(Files,j+1);
      Files[j]:=ExtractFilePath(DblstFileName)+cStrLst[i];
    end; //这里为了保证Files变量原有东西不变，然后增量加入新搜寻出来的文件 
    SetLength(cStrLst,0);//释放
  end;

var
  AFileName_M,AFileName_P:shortString;
  AcStrLst:_cstrLst;
  i,j:Smallint;
begin
  DoPathSep(DirName);

  AFileName_M:=DirName+'market_db\'+ GetDbLstName('market_db'); //'dblst311.lst'
  AFileName_P:=DirName+'publish_db\'+GetDbLstName('publish_db'); //'dblst.lst'
  if TxtScope=M_None_P_All then AFileName_M:='';
  if TxtScope=M_OutPassaway_P_None then AFileName_P:='';
  if TxtScope=M_All_P_None then  AFileName_P:='';
  
  if Length(AFileName_M)>0 then GetTxtFiles(AFileName_M,AcStrLst);
  if Length(AFileName_P)>0 then GetTxtFiles(AFileName_P,AcStrLst);

  for i:=0 to high(AcStrLst) do
  begin
    if (TxtScope = M_OutPassaway_P_All) or (TxtScope =M_OutPassaway_P_None) then //去掉passaway.txt文件
      if Pos('PASSAWAY',UpperCase(AcStrLst[i]))>0 then continue;
    j:=High(Files)+1;
    SetLength(Files,j+1);
    Files[j]:=AcStrLst[i];
  end;
  SetLength(AcStrLst,0);//释放 
end;
...
begin
  ...
  begin
    if ((DirInfo.Attr and FaDirectory) = FaDirectory) and (DirInfo.Name<>'.')
       and (DirInfo.Name<>'..') then
      FolderFileSize(DirName + DirInfo.Name,Size);
    if ((DirInfo.Attr and FaDirectory <> FaDirectory) and
      (DirInfo.Attr and FaVolumeId <> FaVolumeID)) then
...
begin
  ...
  DoPathSep(DirName);
  if (DirName='C:\') or (DirName='C:\\') or
     (DirName='D:\') or (DirName='D:\\') or
     (DirName='E:\') or (DirName='E:\\') or
     (DirName='F:\') or (DirName='F:\\') or
     (DirName='G:\') or (DirName='G:\\') or
     (DirName='H:\') or (DirName='H:\\') or
     (DirName='I:\') or (DirName='I:\\') or
     (DirName='J:\') or (DirName='J:\\') or
     (DirName='K:\') or (DirName='K:\\') or
     (DirName='L:\') or (DirName='L:\\') or
     (DirName='M:\') or (DirName='M:\\') or
     (DirName='N:\') or (DirName='N:\\') Then Exit;
  ...
  begin
    if ((DirInfo.Attr and FaDirectory) = FaDirectory) and (DirInfo.Name<>'.')
       and (DirInfo.Name<>'..') then
      if IncludeSubFolder Then
         DelAllFiles(DirName +  DirInfo.Name);
...
begin
  ...
  begin
    if ((DirInfo.Attr and FaDirectory) = FaDirectory) and (DirInfo.Name<>'.')
       and (DirInfo.Name<>'..') then
      if IncludeSubFolder Then
         Deltree(DirName +  DirInfo.Name,IncludeSubFolder,DelFolderName);
...
Begin
  ...
  Result := reg.KeyExists('SOFTWARE\TraderOne\'+AppName);
  reg.Free;
...
function GetWin_ProxyEnable(var aProxyEnable:boolean;
  var aProxSvr:ShortString;var aProxPort:integer):boolean;
var
  reg : TRegistry;  sTemp,sTemp2:string; iPos:integer;
Begin
  result:=false;
  aProxyEnable:=false;
  aProxSvr:='';
  ...
  try
    ...
    begin
      aProxyEnable:=reg.ReadBool('ProxyEnable');
      if aProxyEnable then
      begin
        sTemp:=reg.ReadString('ProxyServer');
        iPos:=pos(':',sTemp);
        if iPos>0 then
        begin
          aProxSvr:=Copy(sTemp,1,iPos-1);
          sTemp2:=Copy(sTemp,iPos+1,length(sTemp) );
          if MayBeDigital(sTemp2) then
            aProxPort:=strtoint(sTemp2);
        end;
      end;
    end;
    Result:=true;
...
Begin
  ...
  reg := TRegistry.Create;
  reg.RootKey := HKEY_CURRENT_USER;
  reg.OpenKey('SOFTWARE\TraderOne\'+AppName,False);
  Result := reg.ReadString(AppKey);
  reg.CloseKey;
...
Begin
  ...
  reg := TRegistry.Create;
  reg.RootKey := HKEY_CURRENT_USER;
  reg.DeleteKey('SOFTWARE\TraderOne\'+AppName);
  reg.CloseKey;
  reg.Free;
...
Begin
  ...
  reg := TRegistry.Create;
  reg.RootKey := HKEY_CURRENT_USER;
  reg.CreateKey('SOFTWARE\TraderOne\'+AppName);
  reg.OpenKey('SOFTWARE\TraderOne\'+AppName,True);
  reg.WriteString(AppKey,AppValue);
  reg.CloseKey;
  reg.Free;
...
Begin
      Result := false;
      {if FileAge(SFile1) > FileAge(DFile2) then //狦ㄓ方郎ら戳ゑヘ郎穝
         Result := False;}
end;

Function  GetWinSysPath():ShortString;
Var
  NUS : Array[0..254] of Char;
  MPath : Cardinal;
Begin

  MPath := 255;
  NUS   := '';
  IF GetSystemDirectory(NUS,MPath) <> 0 Then
     Result := StrPas(NUS);
...
Begin
  ...
  MPath := 255;
  NUS   := '';
  IF GetTempPath(MPath,NUS) <> 0 Then
     Result := StrPas(NUS);

End;

{Function Get_DbPath(AliasName:ShortString):ShortString;
Var
  v : TDataBase;
Begin
      if Pos('\',AliasName)>0 then
      Begin
         Result := AliasName;
         Exit;
      End;
      v := TDataBase.Create(nil);
      v.AliasName    := AliasName;
      v.DataBaseName := AliasName;
      v.Connected := true;
      Result := v.Directory;
      v.Connected := False;
      v.Destroy;
...
Procedure SetLanguage(value:ShortString);
Begin
     WriteRegStr('Language','value',Value);
End;

function SysisBIG5(): Boolean;
Begin
     Result := False;
     if ReadRegStr('Language','value')<>'GB' then Result := true;
End;
...
Begin
   ...
   if IniLst<>Nil Then
      IniFileRecLst := IniLst;

   if IniFileRecLst = nil Then Exit;

   For i := 0 to IniFileRecLst.Count-1 do
      FreeMem(IniFileRecLst.Items[i]);
   IniFileRecLst.Clear;
...
Function  GetIniCache(SectionName:PChar;
                      ...
                      DefaultS:PChar;IniLst:TList=Nil):ShortString;
Var
  vSectionName : ShortString;
  vKeyName     : ShortString;
  i : Integer;
  aRec  : ^TIniFileRec;
Begin
   ...
   if IniLst<>Nil Then
    ...
    vSectionName := '['+UpperCase(StrPas(SectionName))+']';
    vKeyName := UpperCase(StrPas(KeyName));
    Result := StrPas(DefaultS);
    For i:=0 to IniFileRecLst.Count-1 do
    Begin
         aRec := IniFileRecLst.Items[i];
         if (aRec.SectionName = vSectionName) and
            (aRec.KeyName = vKeyName) Then
         Begin
           Result :=  aRec.ValueStr;
           Break;
         End;
    End;
...
Begin
    ...
    if IniLst=Nil Then
    Begin
      if IniFileRecLst = nil Then
         IniFileRecLst := TList.Create;
    End Else
    Begin
      IniFileRecLst := IniLst;
    End;

    if IniFileRecLst.Count>0 Then
    Begin
        ShowMessage('Please Call FreeIniCache First(TCommon)');
        Exit;
    End;

    fileName := StrPas(iniFile);
    ...
    Begin
        ...
        if (Pos('[',Str)=1) and (Pos(']',Str)=Length(str)) Then
        Begin
           SectionName := UpperCase(Str);
           Continue;
        End;
        aRec  := AllocMem(SizeOf(TIniFileRec));
        aRec.SectionName := SectionName;
        aRec.KeyName  := UpperCase(Copy(Str,1,Pos('=',Str)-1));
        aRec.ValueStr := Copy(Str,Pos('=',Str)+1,Length(Str)-Pos('=',Str));
        IniFileRecLst.Add(aRec);
    End;
    Closefile(f);
...
begin
     Size:=255;
     GetPrivateProfileString(SectionName,keyName,DefaultS,s,size,FileName);
     Result:=s;
end;
...
Begin
    //sep:='/';
    //if Pos('-',DateToStr(Date))>0 then sep:='-';
...
Begin
  Result := '';
  sLst := DoStrArray(Str,Sep);
  For i :=0 to High(sLst) do
      Result := Result + sLst[i];
End;
...
Begin
  ...
  try
     ...
     For i:=0 to Length(Str) do
        //if Pos('、',Str)>0 then
//        begin
//         if sep=Copy(Str,i,2) Then p :=p+1;
//        end else
//          if  sep=Copy(Str,i,1) Then p :=p+1;
        begin
          q:=length(sep);
          if sep=Copy(Str,i,q) then p:=p+1;
        end;
     SetLength(cStr,p+1);
     For i:=0 to p do
        cStr[i] := '';
...
//               begin
//                 if pos('、',str)>0 then
//                  Delete(Str,1,2)  //中文字符占用2个字节 20070705
//                 else
//                   Delete(Str,1,1);  //英文字符占用一个字符
//               end;
        End
        Else
        Begin
...
//CBStrike3Frm2.0.0.0-Doc2.3.0.0-需求6-libing-2007/09/20-修改
//*************************************************************************
Function DoStrArray(Str:ShortString;sep:ShortString): _cStrLst;
Var
   cStr : _cStrLst;
...
{Procedure CreateField(aTable  :TTable; FieldName :ShortString;  FieldSize :Integer; FieldType :TFieldType);
var
   aField:TFieldDef;
Begin
      aField          := aTable.FieldDefs.addFieldDef;
      aField.Name     := FieldName;
      aField.DataType := FieldType;

      if FieldSize>0 then
         aField.Size:=FieldSize;

end;

Procedure CreateIndexField(aTable :TTable; FieldName :ShortString; aFields   :ShortString; aOptions  :TIndexOptions);
var
   aIndexField:TIndexDef;
Begin

     aIndexField         := aTable.IndexDefs.addIndexDef;
     aIndexField.Name    := FieldName;
     aIndexField.Fields  := aFields;
     aIndexField.Options := aOptions;
...
Begin
   if Length(path)=0 then exit;
   if (Copy(path,Length(path),1)<>'\') and
      (Copy(path,Length(path),1)<>'/') Then path := path + '\';
End;
...
Var
  ...
  i     : Integer;
Begin
   StrLst := DoStrArray(Path,'\');
   Path     := '';
   For i:=0 to High(strLst) Do
   Begin
        if Length(strLst[i])=0 Then Continue;
        Path :=  Path + strLst[i] + '\';
...
Begin
  Try
   Result := NoneNum;
   if v<>NoneNum Then
     result := round2(v*Power(10,vPower));
  Except
    Result := NoneNum;
  End;
End;

function GBOffset(value: ShortString): integer;
begin
...
function BIG5Offset(value: ShortString): integer;
begin
  Result := -1;
  if length(value) >= 2 then
  begin
...
function WordToShortString(value: Word): ShortString;
begin
...
begin
     ...
     begin
         ...
         begin
           ...
           begin
             output := output + WordToShortString(GBOrder[offset]);
             inc(idx);
           end
           else
     ...
     begin
       ...
       Else
         ...
         begin
           ...
           begin
             output := output + WordToShortString(BIG5Order[offset]);
             inc(idx);
           end
           else
              output := output + value[idx];
