...
//--
  ...
  public
      ...
      function ProSep_cbstopconv(aFtpXiaShi,aFtpNotXiaShi:boolean):string;  //氨ゎ锣传戳丁 InitBin
      ...
      function ProSepPassHis_cbstopconv(aExtractYear:string):string;  //氨ゎ锣传戳丁 InitBin
...
begin
   ...
   else if  pos('cbthreetrader',TypeFlag)>0  then //如果是三大法人
     SetCBDataLog('DocCenter',FileName,aCBFileName,Operator,aTimeKey,1,'','cbthreetrader@')
   else if  pos('cbdealer',TypeFlag)>0  then  // 自营商
      ...
      FileNameEN:=ExtractFileName(FileName);
       ...
       begin
         ...
         if SameText(ExtractFileName(FileName),'ntd2usd.dat') then
           FileNameCN:= FAppParam.TwConvertStr ('穝籓刽/じ蹲瞯')+ ExtractFileName(FileName)
         else if SameText(ExtractFileName(FileName),'fed.dat') then
           FileNameCN:= FAppParam.TwConvertStr ('fed瞯戈')+ ExtractFileName(FileName)
         else if SameText(ExtractFileName(FileName),'stockq.dat') then
           FileNameCN:= FAppParam.TwConvertStr ('瓣そ杜崔瞯')+ ExtractFileName(FileName)
         else
           FileNameCN:= FAppParam.TwConvertStr ('蹲瞯瞯')+ ExtractFileName(FileName);
       end
...
begin
    ...
    if SameText(DstFileName,'STRIKE4.DAT') or  //--DOC4.4.0.0 pqx 20120207
       ...
       SameText(DstFileName,'CBSALEDATE.DAT') then //--DOC4.0.0N004 huangcq090317 add cbRedeemDate.dat\cbSaleDate.dat
    begin
      sTemp:=Get_GUID8;
...
begin
  ...
  //if not TryToCopyFile((aSrcFileName),(aTmpFile),_TryCpyTimes) then
  //  raise Exception.Create('cpy fail');
...
function TCBDataMgr.ProSep_cbdate(aFtpXiaShi,aFtpNotXiaShi:boolean):string;  //重要日期
var sFile1,sFile2,sFile3:string;
